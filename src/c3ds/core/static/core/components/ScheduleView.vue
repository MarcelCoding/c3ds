...
import {getCurrentTime} from "../ts/ntp.ts";
  ...
  const room_filter_combined: ComputedRef<string[]> = computed(() => {
    ...
    if (props.guid_filter.length > 0) {
      for (let room_guid of props.guid_filter) {
        // A GUID that is not in the current schedule is simply not a filter we can apply.
        const room = guid_rooms.value[room_guid]
        if (room?.name) guid_room_names.push(room.name)
        else console.warn('guid filter references an unknown room:', room_guid)
      }
    }
  ...
  const next_talks: ComputedRef<ProcessedEvent[]> = computed(() => {
    ...
    // Trim only after sorting, otherwise the first room fills every slot.
    return _next_talks.slice(0, max_talks)
  })

  let minuteTimer: number | undefined = undefined
  ...
  onBeforeUnmount(() => {
    if (minuteTimer !== undefined) window.clearTimeout(minuteTimer)
  })
